...
void Canvas::StrokeEllipse(float cx, float cy, float rx, float ry, const D2D1_COLOR_F& c, float w)
{
    if (rx <= 0.5f || ry <= 0.5f) return;
    if (auto* b = Brush(c)) m_dc->DrawEllipse(D2D1::Ellipse(D2D1::Point2F(cx, cy), rx, ry), b, w);
}
